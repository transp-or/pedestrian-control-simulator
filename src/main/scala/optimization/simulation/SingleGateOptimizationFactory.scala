...
class SingleGateOptimizationFactory(config: Config) extends SAProblemsAbstractFactory { // MinFunction est definie que par un seul double

  def createSAProblem(objects: util.List[java.lang.Object]): SAProblem = {

    new SingleGateOptimisation(config, objects.asInstanceOf[java.util.ArrayList[java.lang.Double]])
