...
import hubmodel.{PedestrianDES, PedestrianSim, Position, Time, VertexCell, timePrint, timeReadable}
...
class MovingPedestriansWithDensityVideo(outputFile: String,
                                        ...
                                        criticalAreaInput: List[VertexCell],
  ...
  // Loads the background image which is used as the base image for all dimensions.
    ...
    val cleanCanvas: BufferedImage = bkgdImage match {
      case Some(f) => try { ImageIO.read(new File(f)) } catch {
        case ime: javax.imageio.IIOException => println(ime + " in MakeVideo for file: " + f); createWhiteBackground
        case e : Throwable => println(e + " in MakeVideo for file: " + f); createWhiteBackground }
      case None => { createWhiteBackground }
    }
  ...
  for (i <- times2Show.indices) {
    ...
    if (densityMeasurements.nonEmpty && criticalAreaInput.nonEmpty) {
      val criticalArea: VertexCell = criticalAreaInput.head
