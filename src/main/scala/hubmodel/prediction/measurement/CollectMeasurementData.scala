      ...
      if (nbrPaxInZone > 0) {
        try {
          ...
          zone.paxIndividualDensityHistory.append(
            (sim.currentTime, voronoiTessellations.filter(s => isInVertex(zone)(Vector2D(s.x, s.y))).filter(p => p.getPolygon.getArea.isFinite && !p.getPolygon.getArea.isNaN && p.getPolygon.getArea > 0).map(1.0 / _.getPolygon.getArea))
          )
        } catch {
          case e: Exception => {
