...
import optimization.ALNS.{ALNS, ALNSParameters, DirectionMatchFlow, DownstreamDensityUpdate, FunctionEvaluation, MinimumDurationSameDirection, RandomChangeDirection, RandomDecreaseSpeed, RandomIncreaseAllSpeeds, RandomIncreaseSpeed, RandomSetSpeed, SpeedLowerBound, SpeedUpperBound}
import tools.Time
  ...
  class RollingHorizonOptimization(sim: PedestrianSimulation) extends Action {
    ...
    override def execute(): Any = {
      ...
      val horizonOptimization: ALNS = new ALNS(
        ...
        Vector(RandomIncreaseSpeed, RandomDecreaseSpeed, MinimumDurationSameDirection, RandomIncreaseAllSpeeds, DirectionMatchFlow, DownstreamDensityUpdate, RandomSetSpeed),
