...
class MovingWalkwayAbstract(val name: String, val firstVertex: Vertex, val secondVertex: Vertex, val width: Double, val start: Position, val end: Position, val associatedZonesStart: Vector[Vertex], val associatedZonesEnd: Vector[Vertex], val droppedVertices: Vector[String], val associatedConnectivity: Iterable[MyEdge], val parallelFlows: Vector[Vector[Vertex]]) extends MyEdge(firstVertex, secondVertex) with ControlDeviceComponent {
  ...
  protected  var accStartTime: Time = Time(0)
  protected  var accEndTime: Time =  Time(0)
  protected  val acc: Double = hubmodel.AMW_ACCELERATION_AMPLITUDE // norms prescribes max at 1m/ss but in practice is [0.14,0.43]m/ss (Riccardo's paper).
  protected  var accDirection: Double = 0.0
  protected var _nextSpeed: Double = 0.0
  protected  var _previousSpeed: Double = 0.0
  ...
  protected  val positiveEdge: MyEdge = new MyEdge(this.firstVertex, this.secondVertex)
  protected  val negativeEdge: MyEdge = new MyEdge(this.secondVertex, this.firstVertex)
  ...
  protected  val pedestriansOnAMW: collection.mutable.Set[String] = collection.mutable.Set()
  ...
  protected  val controlPolicy: collection.mutable.PriorityQueue[AMWPolicy] =
    collection.mutable.PriorityQueue.empty[(AMWPolicy)](Ordering.by((_: AMWPolicy).start).reverse)

  protected  val closeTimes: collection.mutable.PriorityQueue[Time] = collection.mutable.PriorityQueue.empty[(Time)](Ordering.by((_: Time).value).reverse)
  protected  val openTimes: collection.mutable.PriorityQueue[Time] = collection.mutable.PriorityQueue.empty[(Time)](Ordering.by((_: Time).value).reverse)
  ...
  /** create a deep copy of the control device
    ...
    * @return
    */
  override def deepCopy: MovingWalkwayAbstract = new MovingWalkwayAbstract(this.name, this.firstVertex, this.secondVertex, this.width, this.start, this.end, this.associatedZonesStart.map(_.deepCopy), this.associatedZonesEnd.map(_.deepCopy), this.droppedVertices, this.associatedConnectivity.map(_.deepCopy), this.parallelFlows)

  def deepCopyWithState(pop: Iterable[PedestrianNOMAD]): MovingWalkwayAbstract = {
    val amw = new MovingWalkwayAbstract(this.name, this.firstVertex, this.secondVertex, this.width, this.start, this.end, this.associatedZonesStart.map(_.deepCopy), this.associatedZonesEnd.map(_.deepCopy), this.droppedVertices, this.associatedConnectivity.map(_.deepCopy), this.parallelFlows)
