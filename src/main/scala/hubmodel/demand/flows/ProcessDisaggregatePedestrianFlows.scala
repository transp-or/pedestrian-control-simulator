...
class ProcessDisaggregatePedestrianFlows[T <: PedestrianNOMAD](eventCollection: Iterable[(String, String, Time)], sim: NOMADGraphSimulator[T])(implicit tag: ClassTag[T]) extends Action {
  ...
  def execute(): Unit = {
    eventCollection
      .filter(ec => ((sim.startTime <= ec._3 && ec._3 <= sim.finalTime) || ec._3.value == math.pow(10, 8)) && ec._1 != "-1" && ec._2 != "-1" && sim.graph.vertexMapNew.keySet.contains(ec._1.drop(2)) && sim.graph.vertexMapNew.keySet.contains(ec._2.drop(2)))
      .foreach(ec => {
