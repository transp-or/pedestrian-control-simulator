...
import hubmodel.demand.{PedestrianFlowFunction_New, PedestrianFlow_New, PedestrianGenerationNonHomogeneousRate, PedestrianGenerationOverInterval, splitFractionsUniform}
...
class ProcessPedestrianFlows[T <: PedestrianNOMAD](pedestrianFlows: Iterable[PedestrianFlow_New], pedestrianFlowsFunction: Iterable[PedestrianFlowFunction_New], sim: NOMADGraphSimulator[T])(implicit tag: ClassTag[T]) extends Action[T] {
  ...
  override def execute(): Unit = {
    ...
    pedestrianFlowsFunction
      .foreach(flow => splitFractionsUniform(sim.stop2Vertices(flow.O), sim.stop2Vertices(flow.D))
        .foreach(f => {
