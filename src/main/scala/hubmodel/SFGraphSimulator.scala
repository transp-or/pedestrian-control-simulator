...
import hubmodel.input.infrastructure.{BinaryGate, GraphReader, NodeNaming, RouteGraph, SocialForceSpace, StartFlowGates}
...
class SFGraphSimulator(override val startTime: Time,
                       ...
                       val nodeNaming: NodeNaming) extends PedestrianDES[PedestrianSim](startTime, finalTime) {
  ...
  val useFlowGates: Boolean = graph.flowGates.nonEmpty

  /** Indicator whether binary gaets are present */
  val useBinaryGates: Boolean = graph.binaryGates.nonEmpty

  /** KPIs */
  val criticalArea: List[Vertex] = List(Vertex("CriticalZone1", DenseVector(50.0, 10.720), DenseVector( 56.0, 10.720), DenseVector( 56.0, 16.800), DenseVector( 50.0, 16.800)))
  ...
  class StartSim(sim: SFGraphSimulator) extends super.GenericStartSim(sim) {
    override def execute(): Unit = {
      ...
      //println(sim.pedestrianFlows.flows.map(f => (f.start.toSecondOfDay, f.end.toSecondOfDay)))
      ...
      sim.insertEventWithDelay(0)(new EvaluateState(sim))
