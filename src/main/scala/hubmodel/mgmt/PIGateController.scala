...
class PIGateController(sim: SFGraphSimulator) extends Action {
  ...
  override def execute(): Unit = {
    ...
    val totalInflow: Double = max(0.1,min(3.0, sim.inflowHistory.last._2 - 8.5*(sim.densityHistory.last._2 - sim.densityHistory.dropRight(1).last._2) + 0.75*(0.8 - sim.densityHistory.last._2)))
