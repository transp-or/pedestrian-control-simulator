...
object RunSimulation extends App with StrictLogging {
  ...
  if (resultsJson.nonEmpty) {
    ...
    if (config.getBoolean("output.travel-time.mean-by-intervals")) {
      val intervals = (simulationStartTime.value to simulationEndTime.value by 15.0).map(_.toDouble).toVector
      def findTimeInterval(v: Double) = findInterval(intervals)(v)
      resultsJson.flatMap(r => {
        val demand = r.tt.groupBy(v => findTimeInterval(v.entry)._1).view.mapValues(_.size).toVector.toMap
        val tt = r.tt.filter(_.exit.isDefined).groupBy(v => findTimeInterval(v.exit.get)._1).view.mapValues(g => g.map(_.tt).sum/g.size).toVector.toMap
        intervals.sliding(2).map(v => (v(0) + v(1))/2).zipWithIndex.map(i => (i._1, demand.getOrElse(i._2,0), tt.getOrElse(i._2,0)))
