...
class SingleDayAggregateProcessor(fileName: String,
  ...
  private def computeVoronoiDiagrams(z: NewZone, times: Seq[Double], voronoiBoundary: PolygonSimple): Vector[(Double, Seq[(DenseVector[Double], PolygonSimple, Site)])] = {
      ...
      if (p._2.nonEmpty) {
        ...
        p._2.foreach(id => {
          ...
          if (idx >=0 && idx < ped(id).h_t.size-1) {
            ...
            if(z.isInside(pos)) {stupidList.add(new Site(pos._1, pos._2))}
          }
    ...
    (res ++ (for (t <- times if !res.exists(_._1 == t)) yield {(t, Seq())}).toVector).sortBy(_._1)
  }
 ...
 /* def computeInflowOutflow(mainZone: Zone, times: Seq[Double]): Vector[(Double, Double, Int, Int)] = {
  ...
  def computeFlowsNew(mainZone: Zone, zones: Vector[Zone], times: Vector[Double]): Map[(Int, Int, Int), Int] = {
   ...
   def findZone(x: Double, y: Double): Int = {
    ...
    val pedZonesTimes: Iterable[Vector[(Double, Int)]] = this.ped.values.map(p => {p.h_t.zip(p.h_x.zip(p.h_y).map(t => findZone(t._1, t._2))).toVector})
  ...
  def computeSideOfLine(point: (Double, Double), start: (Double, Double), end: (Double, Double)): SideOfLine = {
    ...
    if (value > 0.0) { ONLEFT }
    else if (value < 0.0) { ONRIGHT }
    else { ONRIGHT } // if on the line, consider it as being inside
  }
  ...
  def computeFlowsAgain(lines: Vector[FlowLineDA], times: Vector[Double]): (Map[Int, Int], Map[Int, Int], Map[Int, Int]) = {
    ...
    (controlledInflow ++ (for (i <- times.indices.dropRight(1)) yield { i -> controlledInflow.getOrElse(i, 0) }).toMap,
    uncontrolledInflow ++ (for (i <- times.indices.dropRight(1)) yield { i -> uncontrolledInflow.getOrElse(i, 0) }).toMap,
    outflow ++ (for (i <- times.indices.dropRight(1)) yield { i -> outflow.getOrElse(i, 0) }).toMap)
  }
