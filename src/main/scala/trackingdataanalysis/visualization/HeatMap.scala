...
class HeatMap(outputFile: String,
  ...
  def minSkipNaN(data: Iterable[Double]): Double = {
    data.filterNot(x => x.isNaN || x.isInfinity).min
  }

  def maxSkipNaN(data: Iterable[Double]): Double = {
    data.filterNot(x => x.isNaN || x.isInfinity).max
  }
