...
class HeatMap(outputFile: String,
  ...
  val xInterval: Double = {
    val xCoordsDistinct: Vector[Double] = data.map(_._1).toVector.distinct.sorted
    val distinctGaps: Vector[Double] = xCoordsDistinct.dropRight(1).zip(xCoordsDistinct.tail).map(t => 100.0*(t._2 - t._1)).distinct.map(_/100.0)
  ...
  drawHeatMap(gCanvas, data, (xInterval, data.map(_._1).toVector.distinct.size), (yInterval, data.map(_._2).toVector.distinct.size))
  drawColorMapLegend(gCanvas, data.map(_._3).min, data.map(_._3).max, label)
