...
object AnalyseTrackingData extends App {
  ...
  processedData match {
    case data: SingleDayAggregateProcessor => {
      ...
      val lengthControlledInflow: Double = monitoredZone.flowLines.filter(_.inflowType == CONTROLLED).map(fl => norm(fl.line._1, fl.line._2)).sum//7.37//1.0/2.0
      ...
      val inflowControlled: Array[Double] = flowsAgain._1.toVector.sortBy(_._1).map(_._2/(dt*lengthControlledInflow)).toArray//flowArray.map(_(2))
      val inflowUncontrolled = flowsAgain._2.toVector.sortBy(_._1).map(_._2/(dt*lengthUncontrolledInflow)).toArray//flowArray.map(_(1))
